...
import * as searchStudentAction from './actions';
import SpinnerComponent from '../../uicomponents/spinner/spinner.component';
import * as appUtils from '../../utils/app-utils';
import './search-student.component.scss';

/**
 * Defines layout of Search Master Student Screen
 */
 ...
 */
  idFormatter(cell, row) { // eslint-disable-line no-unused-vars
    return (
      <Link to={`/student/${cell}`}>{cell}</Link>
    );
  }
        ...
        <div className="table">
          <div className="row">
            <div className="col-md-4">
              ...
              <h4 className="pageHeaders">Search Student</h4>
            </div>
          </div>
        </div>
...
SearchStudentComponent.propTypes = {
  actions: PropTypes.object.isRequired,
  data: PropTypes.array.isRequired,
  loading: PropTypes.bool,
};

SearchStudentComponent.contextTypes = {
  router: PropTypes.object.isRequired,
};

const mapStateToProps = store => (
  {
    data: store.searchStudentReducer.data,
    loading: store.searchStudentReducer.loading,
  }
);
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(searchStudentAction, dispatch),
  };
}
